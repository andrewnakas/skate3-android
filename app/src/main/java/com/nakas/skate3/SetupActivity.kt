...
class SetupActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val end = if (killed) GameData.lastSessionEnd(this) else SessionEnd.NORMAL
        GameData.markSessionRunning(this, false)
        refresh()
        // Only a real fault is worth a dialog. See explainCrashedSession.
        if (end == SessionEnd.NATIVE_CRASH || end == SessionEnd.APP_CRASH) {
            explainCrashedSession(end)
        }
    }

    /**
     * Says so when the last session ended in a real fault, and stays quiet
     * otherwise.
     *
     * Every abnormal ending used to raise a dialog. That meant backgrounding
     * the game - which on a process this size invites Android to reclaim it -
     * greeted the player with "The game was closed by Android" the next time
     * they opened the app, over and over, for something that is ordinary,
     * unavoidable and already explained the first time. Being told is useful
     * once; being told every launch is a nag, and it trains people to dismiss
     * the box without reading it, which costs exactly the case that matters.
     *
     * So only a genuine crash speaks up. Nothing is lost by the silence: the
     * exit reason and Android's own detail go into "Copy the details"
     * regardless, which is where a report is built from anyway - so a memory
     * kill, an ANR or a force-stop is still fully diagnosable after the fact
     * without interrupting anyone to say it happened.
     */
    private fun explainCrashedSession(end: SessionEnd) {
        val detail = GameData.lastSessionEndDetail(this)
        val body =
            "The last session ended in a fault rather than Android reclaiming memory, " +
                "so it is worth reporting rather than working around.\n\n" +
                "Use \"Copy the details\" below and open an issue with it. If it happens " +
                "every time you start the game, try selecting the System GPU driver " +
                "first: a custom driver that cannot run on this device fails exactly " +
                "like this.\n\n" +
                "Your career progress is saved by the game as you play."
        AlertDialog.Builder(this)
            .setTitle(if (end == SessionEnd.NATIVE_CRASH) "The game crashed" else "The app crashed")
            .setMessage(body + if (detail.isBlank()) "" else "\n\n$detail")
